...
import (
	...
	"gitlab.com/sibsfps/spc/spc-1/daemon/workersd/api"
...
func (s *Server) Initialize(cfg config.Local) error {
	...
	serverNode = api.APINode{WorkersNode: thisNode}
...
func (s *Server) Start() {
	s.log.Info("Trying to start a Workers node")
	s.node.Start()
	s.log.Info("Successfully started a Workers node.")
	cfg := s.node.Config()
